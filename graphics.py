...
print(f'Drawing {PARTICLE_NUM} particles trajectories...')
...
if U_FUNCTION == 2:
    ...
    U_wind = -(X_wind**2 + Y_wind**2)**0.5 * Y_wind
    V_wind = (X_wind**2 + Y_wind**2)**0.5 * X_wind

ax.streamplot(X_wind, Y_wind, U_wind, V_wind, color='gray', density=5, linewidth=1, arrowsize=1.5)

# Plot particle trajectories
for i in range(PARTICLE_NUM):
    print(f"\r{i/PARTICLE_NUM*100:.1f}%", end="")
...
print("\r100.0%")
print(f"Particle trajectories graphic saved as wind_field_{U_FUNCTION}_particles_{PARTICLE_NUM}.png!")
